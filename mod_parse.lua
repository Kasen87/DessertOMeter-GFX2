--[[
Parse module for Corona SDK v1.2
Copyright (c) 2013 develephant
...
function Parse:linkFile( parseObjectType, parseObjectId, linkField, parseFileUriToLink, _callback )
  local linkField = linkField
  local fileDataTable = { [linkField] = { ["name"] = parseFileUriToLink, ["__type"] = "File" } }
  if parseObjectType == Parse.USER then
    return self:updateUser( parseObjectId, fileDataTable, _callback )
  else
    return self:updateObject( parseObjectType, parseObjectId, fileDataTable, _callback )
    ...
    if self.showStatus then
      ...
      if decodedResponse.code then
        ...
        if decodedResponse.error then
          msg = msg .. "Sorry, there was a problem: " .. decodedResponse.error
          header = "Error"
        end
      else
        msg = "Success!"
      ...
      native.showAlert( header, msg , { "OK" } )
